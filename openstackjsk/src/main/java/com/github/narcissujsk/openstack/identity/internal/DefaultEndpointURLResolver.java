package com.github.narcissujsk.openstack.identity.internal;
...
public class DefaultEndpointURLResolver implements EndpointURLResolver {
    ...
    private String resolveV2(URLResolverParams p) {
        ...
        if (!services.isEmpty()) {
            ...
            for (com.github.narcissujsk.model.identity.v2.Endpoint ep : sc.getEndpoints()) {
    ...
    private String resolveV3(URLResolverParams p) {
        ...
        for (com.github.narcissujsk.model.identity.v3.Service service : token.getCatalog()) {
            if (p.type == ServiceType.forName(service.getType()) || p.type == ServiceType.forName(service.getName())) {
                if (p.perspective == null) {
                    p.perspective = Facing.PUBLIC;
                }
                for (com.github.narcissujsk.model.identity.v3.Endpoint ep : service.getEndpoints()) {
                    if (matches(ep, p)) {
     ...
     * @return
     */
    private boolean matches(com.github.narcissujsk.model.identity.v3.Endpoint endpoint, URLResolverParams p) {
